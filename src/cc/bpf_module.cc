...
int BPFModule::annotate_prog_tag(const string &name, int prog_fd,
  ...
  if (err)
    return err;
  err = bpf_prog_get_tag_from_fd(prog_fd, &tag2);
  if (err)
    return err;
  if (tag1 != tag2) {
  ...
  err = mkdir("/var/tmp/bcc", 0777);
  if (err && errno != EEXIST) {
    fprintf(stderr, "cannot create /var/tmp/bcc\n");
    return -1;
  }

  char buf[128];
  ::snprintf(buf, sizeof(buf), "/var/tmp/bcc/bpf_prog_%llX", tag1);
  err = mkdir(buf, 0777);
  if (err && errno != EEXIST) {
  ...
  ::snprintf(buf, sizeof(buf), "/var/tmp/bcc/bpf_prog_%llX/%s.rewritten.c",
             tag1, name.data());
